...
def main():
    ...
    bilstm_model.fit(X_train_cv, y_train_cv, epochs=5, batch_size=64, validation_split=0.2)

    bilstm_loss, bilstm_accuracy = bilstm_model.evaluate(X_test_cv, y_test_cv)
    bilstm_auc = roc_auc_score(y_test_cv, bilstm_model.predict(X_test_cv))

    print(f"BiLSTM Accuracy: {bilstm_accuracy}")
    print(f"BiLSTM AUC: {bilstm_auc}")

    bilstm_model.save(args.savemodel)

    print("###---EVALUATION-TEST")

    predicted_probabilities = bilstm_model.predict(X_test_cv)
