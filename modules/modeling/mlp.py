...
def main():
    ...
    mlp_model.fit(X_train_cv.reshape(len(X_train_cv), -1), y_train_cv, epochs=10, batch_size=64, validation_split=0.2)

    mlp_loss, mlp_accuracy = mlp_model.evaluate(X_test_cv.reshape(len(X_test_cv), -1), y_test_cv)
    mlp_auc = roc_auc_score(y_test_cv, mlp_model.predict(X_test_cv.reshape(len(X_test_cv), -1)))

    print(f"MLP Accuracy: {mlp_accuracy}")
    print(f"MLP AUC: {mlp_auc}")

    mlp_model.save(args.savemodel)

    print("###---EVALUATION-TEST")

    predicted_probabilities = mlp_model.predict(X_test_cv.reshape(len(X_test_cv), -1))
