...
def main():
    ...
    model.fit(X_train_cv, y_train_cv, validation_split=0.2, epochs=5, batch_size=64)

    loss, accuracy = model.evaluate(X_test_cv, y_test_cv)
    auc = roc_auc_score(y_test_cv, model.predict(X_test_cv))

    print(f"Transformer Accuracy: {accuracy}")
    print(f"Transformer AUC: {auc}")

    model.save(args.savemodel)

    print("###---EVALUATION-TEST")

    predicted_probabilities = model.predict(X_test_cv)
